...
def prompt(model, message, temperature=0, max_tokens=4096, frequency_penalty=0, presence_penalty=0):
	"""Prompt the ChatGPT models

	Keyword arguments:
	model -- The language model to prompt
	message -- The message send to the language model
	temperature -- Optional parameter that changes the randomness of the language model. Default is 0
	top_P -- Optional parameter that changes the threshold when nucleus sampling. Default is 1
	max_tokens -- Optional parameter that changes the max number of tokens that language model can generate. Default is 4096
	frequency_penalty -- Optional parameter that changes the behaviour of the language model. Default value is 0 i.e. positive value discourages the model from generating commonly used words and negative value encourage the model to stick to more common words or phrases.
	presence_penalty -- Optional parameter that changes the behaviour of the language model. Default value is 0 i.e. positive value reward the model for producing certain tokens, encouraging it to generate these more often and vice versa.
	"""
